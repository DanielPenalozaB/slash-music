...
@SecurityRequirement(name = "bearerAuth") // <- Protege todos los endpoints con esquema bearer (JWT) en Swagger
@RestController
...
@Operation(
        summary = "Listar/Buscar canciones",
        description = "Paginar y filtrar por título o artista. Si no hay filtros, lista todo."

)
@GetMapping
    public Page<SongResponse> list(
            @Parameter(description = "Filtro por título (contiene, ignore case)")
            @RequestParam(required = false) String title,
            @Parameter(description = "Filtro por artista (contiene, ignore case)")
            @RequestParam(required = false) String artist,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @Parameter(description = "Campo de orden: title, artist, createdAt, etc.")
